...
class GestureAHLNet(nn.Module):
    ...
    def __init__(self, hog_dim=1200, acc_dim=53, num_classes=14):
        super().__init__()
        # HOG subnetwork
        self.hog_ahl1 = AdaptiveHiddenLayer(hog_dim, 64, num_groups=2)
        self.hog_ahl2 = AdaptiveHiddenLayer(64,      128, num_groups=2)

        # Accelerometer subnetwork
        self.acc_ahl1 = AdaptiveHiddenLayer(acc_dim, 64, num_groups=2)
        self.acc_ahl2 = AdaptiveHiddenLayer(64,      128, num_groups=2)

        # Joint subnetwork
        self.joint_ahl = AdaptiveHiddenLayer(256, 256, num_groups=2)
...
def main():
    ...
    # LR decay by factor 0.5 every 10 epochs
